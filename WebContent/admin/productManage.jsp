...
<script type="text/javascript">
	    ...
	    $ ("#bName").combobox (
	            {
		            ...
		            {
			            //sName是所属小类的 Id值
			            $ ("#sName").combobox ("reload",
			                    "productSmallType_comboList.action?s_productSmallType.bigType.id=" + record.id);
    ...
    function setProductHot ()
    {
	    ...
	    $.messager.confirm ("系统提示", "您确认要设置这<font color=red>" + selectedRows.length + "</font>个商品为热卖吗？", function (r)
	    {
		    if (r)
		    {
			    $.post ("product_setProductWithHot.action",
			    {
				    ids : ids
			    }, function (result)
			    {
    ...
    function setProductSpecialPrice ()
    {
	    ...
	    $.messager.confirm ("系统提示", "您确认要设置这<font color=red>" + selectedRows.length + "</font>个商品为特价吗？", function (r)
	    {
		    if (r)
		    {
			    $.post ("product_setProductWithSpecialPrice.action",
			    {
				    ids : ids
			    }, function (result)
			    {
...
<body style="margin: 1px;">
	<table id="dg" title="商品管理" class="easyui-datagrid" fitColumns="true" pagination="true" rownumbers="true" url="product_list.action"
		fit="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true" align="center"></th>
				<th field="id" width="50" align="center">编号</th>
				<!--formatter格式化当前列 （默认有两个参数 一个是 当前值（val） 还有一个是当前行（row） ） -->
	...
	<div id="tb">
		...
		<div>
			&nbsp;商品名称：&nbsp;
			<input type="text" id="s_productName" size="20" onkeydown="if(event.keyCode==13) searchProduct()" />
	...
	<div id="dlg" class="easyui-dialog" style="width: 600px; height: 450px; padding: 10px 20px" closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post" enctype="multipart/form-data">
			<table cellspacing="8px">
				<tr>
					<td>商品名称：</td>
					<td colspan="4"><input type="text" id="name" name="product.name" class="easyui-validatebox" required="true"
				...
				<tr>
					<td>所属大类：</td>
					<td colspan="4"><input class="easyui-combobox" id="bName" name="product.bigType.id"
							data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'productBigType_comboList.action'" /></td>
				</tr>
				<tr>
					<td>所属小类：</td>
					<td colspan="4"><input class="easyui-combobox" id="sName" name="product.smallType.id"
							data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'productSmallType_comboList.action'" />
					</td>
				</tr>
				<tr>
					<td valign="top">备注：</td>
					<td colspan="4"><textarea rows="5" cols="50" id="description" name="product.description"></textarea> <input type="hidden"
							id="proPic" name="product.proPic" /> <input type="hidden" id="hot" name="product.hot" /> <input type="hidden" id="hotTime"
							name="product.hotTime" /> <input type="hidden" id="specialPrice" name="product.specialPrice" /> <input type="hidden"
